...
class Controller(object):
    ...
    def control(self, current_velocity, dbw_enabled, linear_velocity, angular_velocity):
        ...
        # rospy.logwarn("Linear Velocity: {0}".format(linear_velocity))
        # rospy.logwarn("Angular Velocity: {0}".format(angular_velocity))
        # rospy.logwarn("Current Velocity: {0}".format(current_velocity))
        ...
        if linear_velocity == 0. and current_velocity < 0.1:
            throttle = 0
            brake = 400 # N/m, change to 700 for real driving
